...
import { X, Trash2, AlertTriangle, Volume2 } from "lucide-react";
import Cookies from "js-cookie";

function ModalDeleteVoice({ isOpen, onClose, voice, onConfirm }) {
  ...
  async function handleDelete() {
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_APP_BACKEND_URL}projects/delete-voice`,
        {
          method: "POST",
          headers: {
      ...
      if (response.ok && data.success) {
        ...
        console.error("Error deleting voice");
      }
    } catch (error) {
        ...
        <div className="px-6 pb-6">
          <div className="mb-6">
            ...
            <div className="bg-[#DC569D30] bg-opacity-10 border border-primarioLogo border-opacity-30 rounded-lg p-4">
              <div className="flex gap-3">
                <Volume2 className="w-5 h-5 text-[#DC569D] flex-shrink-0 mt-0.5" />
                <div>
                  <p className="text-[#DC569D] text-sm montserrat-medium mb-1">
                    This action cannot be undone
                  </p>
                  <p className="text-[#DC569D] text-sm montserrat-regular">
                    The voice and all its associated data will be permanently
          ...
          {voice && (
            <div className="mb-6">
              <div className="flex items-center gap-4 p-4 bg-darkBoxSub rounded-lg border border-gray-600">
                <div className="w-16 h-16 bg-[#F2D543] bg-opacity-20 rounded-lg flex items-center justify-center">
                  <Volume2 className="w-6 h-6 text-primarioDark" />
                </div>
          ...
          <div className="flex gap-3 justify-end">
            <button
              onClick={onClose}
              disabled={isDeleting}
              className="px-4 py-2 border border-gray-600 text-gray-300 rounded-lg hover:bg-gray-700 hover:text-white transition-colors montserrat-regular disabled:opacity-50 disabled:cursor-not-allowed"
            >
              Cancel
            </button>
            <button
              ...
              disabled={isDeleting}
              className="px-4 py-2 bg-primarioLogo text-white rounded-lg hover:bg-[#DC569D50] transition-colors montserrat-medium flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
            >
...
export default ModalDeleteVoice;
