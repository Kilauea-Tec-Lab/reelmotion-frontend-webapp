const translations = {
  en: {
    // Navbar
    "nav.features": "Features",
    "nav.ai-agent": "AI Agent",
    "nav.editor": "Editor",
    "nav.pricing": "Pricing",
    "nav.integrations": "Integrations",
    "nav.developers": "Developers",
    "nav.teams": "Teams",
    "nav.get-started": "Get Started",
    "nav.go-to-dashboard": "Go to Dashboard",

    // Hero
    "hero.badge": "Kling V3 · Seedance 2.5 · Veo 3.1 — Now Live",
    "hero.title": "Launch Your Next Ad Campaign — Before Lunch.",
    "hero.subtitle":
      "Brief it like you would a creative team. Reelmotion AI writes the prompt, picks the model and produces studio-quality video, images and voice for your campaigns in minutes — one variation or fifty. Pay only for what you generate. No subscription, ever.",
    "hero.cta-primary": "Start Your Campaign",
    "hero.cta-secondary": "See It in Action",
    "hero.note": "No subscriptions · No seats · Pay per use — 20 free tokens to start",
    "hero.go-to-dashboard": "Go to Dashboard",

    // Social Proof
    "social-proof.assets-generated": "Campaign Assets Generated",
    "social-proof.ai-models": "AI Models, One Platform",
    "social-proof.hours-value": "Hours",
    "social-proof.hours-label": "Not Weeks",
    "social-proof.countries": "Countries Served",
    "social-proof.variations": "Variations per Brief",

    // Features
    "features.title":
      "Your Whole Team Can Ship Video — No Creative Bottleneck",
    "features.subtitle":
      "Most AI video tools assume you have a prompt engineer on staff. Reelmotion AI doesn't. Our agent turns any team member's brief into a production-grade prompt and delivers assets that look agency-made — so campaigns don't stall waiting on a specialist, and don't stall at volume either. Need one hero video or twenty ad variations, the process is exactly as fast.",
    "features.video.title": "Turn a campaign brief into a full set of video assets",
    "features.video.description":
      "Describe the spot, ad, or social clip you need. Our AI agent writes the prompt and lets you pick from the best models — Seedance 2.5, Veo 3.1, Kling V3, Runway 4.5, and more. Generate multiple versions or formats in parallel, ready for testing across channels.",
    "features.image.title": "On-brand visuals for every channel",
    "features.image.description":
      "Type what the creative needs to look like. Our AI turns it into a precise prompt and generates 4K images with Nano Banana 2, GPT Image 2, or Seedream 5.0.",
    "features.audio.title": "Broadcast-ready voiceover, no studio booking",
    "features.audio.description":
      "Write your script, pick a voice, hit generate. ElevenLabs multi-language TTS delivers agency-quality narration for every market you sell into.",

    // ReelBot
    "reelbot.title": "Your On-Demand Creative Team",
    "reelbot.subtitle":
      "No prompt engineering, no creative brief back-and-forth. Our AI asks the right questions, builds the perfect prompt, and generates results ready to run — so your team can move from concept to campaign the same day.",
    "reelbot.feat.imageGen": "Image Generation",
    "reelbot.feat.imageGenDesc":
      "Text-to-image and image-to-image with Nano Banana 2, GPT Image 2, and Seedream",
    "reelbot.feat.videoGen": "Video Generation",
    "reelbot.feat.videoGenDesc":
      "10 video models: Seedance 2.5, Veo 3.1, Runway 4.5, Kling V3, and more",
    "reelbot.feat.tts": "Text-to-Speech",
    "reelbot.feat.ttsDesc": "Multi-language voices powered by ElevenLabs",
    "reelbot.feat.vision": "Visual Analysis",
    "reelbot.feat.visionDesc":
      "Drop an image or video and our AI understands it instantly — no manual descriptions needed",
    "reelbot.feat.prompts": "Guided Prompt Builder",
    "reelbot.feat.promptsDesc":
      "Our AI asks one question at a time to build the exact prompt that nails the result you want — zero guesswork",
    "reelbot.step1.title": "Brief your campaign idea in your own words",
    "reelbot.step1.desc":
      "No prompt knowledge needed. Just tell our AI what you're imagining.",
    "reelbot.step2.title": "ReelBot crafts your prompt",
    "reelbot.step2.desc":
      "The agent asks smart follow-up questions, then writes a production-grade prompt for you.",
    "reelbot.step3.title": "Pick your model & settings",
    "reelbot.step3.desc":
      "Choose your AI model, resolution, aspect ratio, and duration — all in one place.",
    "reelbot.step4.title": "See the cost before you commit budget",
    "reelbot.step4.desc":
      "Full transparency — exact token cost shown upfront, no surprises.",
    "reelbot.step5.title": "Generate & deliver",
    "reelbot.step5.desc":
      "Your content is ready in seconds — download it or send it straight to the editor for final polish.",

    // Chat Demo
    "demo.title": "No Learning Curve. Just Results.",
    "demo.subtitle":
      "We guide you through the entire process to ensure you get the result that you need, regardless of your experience.",
    "demo.caption":
      "From rough idea to polished content — watch our AI agent do the work",
    "demo.showcaseTitle": "Create with Reelmotion AI",
    "demo.showcaseAlt": "AI-generated content",

    // Editor
    "editor.title":
      "From First Draft to Client-Ready — All In One Tab.",
    "editor.subtitle":
      "A full professional-grade video editor built into the same platform where you generate content. No exporting to a separate tool, no app-switching between your team and freelancers.",
    "editor.feat.timeline": "Multi-track Timeline",
    "editor.feat.timelineDesc":
      "Drag, drop, and zoom frame-by-frame — full precision editing at your fingertips",
    "editor.feat.captions": "Text & Captions",
    "editor.feat.captionsDesc":
      "Animated text overlays, subtitles, and 9+ overlay styles ready to use",
    "editor.feat.overlays": "Image & Video Layers",
    "editor.feat.overlaysDesc":
      "Stack clips, images, shapes, stickers, and captions on separate tracks",
    "editor.feat.audio": "Audio Tracks",
    "editor.feat.audioDesc":
      "Layer music, voiceovers, and sound effects with independent volume control",
    "editor.feat.stock": "Unlimited Stock Media",
    "editor.feat.stockDesc":
      "Millions of royalty-free images and videos via Pexels — built right in",
    "editor.feat.cloud": "Video Rendering",
    "editor.feat.cloudDesc":
      "Export in high quality, rendered in the cloud — your laptop does zero heavy lifting",
    "editor.feat.autosave": "Auto-save & Projects",
    "editor.feat.autosaveDesc":
      "Every change saved automatically. Pick up any project exactly where you left off.",
    "editor.feat.browser": "Zero Install",
    "editor.feat.browserDesc":
      "Works on any device, any browser — open a tab and start editing",

    // Pricing
    "pricing.title": "No Subscriptions. Pay Only for What You Use.",
    "pricing.subtitle":
      "No monthly plans, no seats, no commitments. Buy tokens when you need them and spend them on any model — you see the exact cost of every generation before you hit generate.",
    "pricing.payg.name": "Pay as you go",
    "pricing.payg.badge": "No subscription",
    "pricing.payg.rate": "1 token = US$0.01 · same price for every team size",
    "pricing.payg.min": "Top up from $6 · no minimum monthly spend",
    "pricing.payg.cta": "Start free",
    "pricing.payg.free": "20 free tokens on signup · no credit card",
    "pricing.payg.included": "Everything included",
    "pricing.includes.video": "Video generation",
    "pricing.includes.image": "Image generation",
    "pricing.includes.voice": "Voiceover & TTS",
    "pricing.includes.models": "All 15+ AI models",
    "pricing.includes.editor": "Cloud video editor",
    "pricing.includes.library": "Shared media library",
    "pricing.includes.agent": "AI Agent (ReelBot)",
    "pricing.includes.api": "REST API & MCP server",
    "pricing.example.video": "5s video · Kling V3 720p",
    "pricing.example.image": "1 image · Nano Banana 2",
    "pricing.example.voice": "1,000-char voiceover · ElevenLabs",
    "pricing.example.note":
      "Real prices from the app. Failed generations are refunded automatically.",
    "pricing.enterprise.title": "Running campaigns at scale?",
    "pricing.enterprise.subtitle":
      "Custom token volume and integration support for agencies and brands producing hundreds of assets a month.",
    "pricing.enterprise.bullet1": "Custom token volume",
    "pricing.enterprise.bullet2": "API & MCP integration support",
    "pricing.enterprise.bullet3": "Direct line to the team",
    "pricing.enterprise.cta": "Talk to sales",

    // CTA Banner
    "cta.title": "Your next campaign is one brief away.",
    "cta.subtitle":
      "Join marketing teams and agencies shipping high volumes of video — fast, without the production overhead, and without a subscription.",
    "cta.button": "Start Your Campaign",
    "cta.note": "No credit card. No subscription. 20 free tokens — then pay only for what you use.",

    // Integrations
    "integrations.badge": "✦ Integrations",
    "integrations.title": "Connect Reelmotion to Claude, ChatGPT, or Your Own AI Agent",
    "integrations.subtitle":
      "Reelmotion ships a remote MCP server and a REST API. Connect it once with OAuth and let your agent brief, generate, and deliver campaign assets — no SDK, no glue code.",
    "integrations.logos-caption": "Works with",
    "integrations.any-agent": "Any MCP client",
    "integrations.step1.title": "Connect in one click",
    "integrations.step1.desc":
      "Add Reelmotion as a connector in Claude, ChatGPT, Claude Code or Cursor. OAuth 2.1 with PKCE handles auth — or use an API key.",
    "integrations.step2.title": "Brief from your agent",
    "integrations.step2.desc":
      "Ask for \"20 vertical cuts for TikTok\" — your agent calls create_video, create_image and create_audio with the right model and settings.",
    "integrations.step3.title": "Assets land in your library",
    "integrations.step3.desc":
      "Every generation appears in your Reelmotion library and cloud editor, ready to polish and publish.",
    "integrations.cta": "Read the developer docs",
    "integrations.note": "MCP · OAuth 2.1 + PKCE · REST API · 1 token = US$0.01",

    // Teams
    "teams.badge": "✦ Built for teams",
    "teams.title": "One Workspace for Your Whole Campaign",
    "teams.subtitle":
      "Keep every asset in one place and let teammates and agents pull from the same library.",
    "teams.folders.title": "Shared project folders",
    "teams.folders.desc":
      "Share a folder with teammates so everyone works from the same campaign assets — no re-sending files.",
    "teams.library.title": "Shared media library",
    "teams.library.desc":
      "Every image, video and voiceover generated in a shared folder is available to everyone in it, in the library and the editor.",
    "teams.pipeline.title": "Agent-ready pipeline",
    "teams.pipeline.desc":
      "Plug Reelmotion into your own agents and workflows via MCP or the REST API and generate campaign assets at scale.",

    // Auth Modal
    "auth.tab.signin": "Sign In",
    "auth.tab.register": "Register",
    "auth.signin.title": "Welcome Back",
    "auth.signin.subtitle": "Enter your credentials to access your account",
    "auth.signin.email-placeholder": "Email or username",
    "auth.signin.password-placeholder": "Password",
    "auth.signin.forgot-password": "Forgot password?",
    "auth.signin.button": "Sign In",
    "auth.signin.email-required": "Email is required",
    "auth.signin.password-required": "Password is required",
    "auth.signin.invalid-credentials": "The email or username is incorrect",
    "auth.register.title": "Create Account",
    "auth.register.subtitle": "Join the new era of content creation",
    "auth.register.username-placeholder": "Username",
    "auth.register.name-placeholder": "Full Name",
    "auth.register.email-placeholder": "Email Address",
    "auth.register.password-placeholder": "Password",
    "auth.register.confirm-password-placeholder": "Confirm Password",
    "auth.register.accept-terms-prefix": "I accept the",
    "auth.register.terms": "Terms & Conditions",
    "auth.register.and": "and",
    "auth.register.privacy": "Privacy Policy",
    "auth.register.accept-terms-error":
      "You must accept the Privacy Policy and Terms & Conditions to create an account",
    "auth.register.email-invalid": "Please enter a valid email address",
    "auth.register.password-empty": "Password fields cannot be empty",
    "auth.register.password-mismatch": "Passwords do not match",
    "auth.register.button": "Create Account",
    "auth.reset.title": "Reset Password",
    "auth.reset.subtitle":
      "Enter your email address and we'll send you instructions to reset your password.",
    "auth.reset.email-label": "Email Address",
    "auth.reset.email-placeholder": "Enter your email",
    "auth.reset.email-required": "Email is required",
    "auth.reset.email-invalid": "Please enter a valid email address",
    "auth.reset.network-error": "Network error. Please try again.",
    "auth.reset.fallback-success":
      "Password reset instructions have been sent to your email.",
    "auth.reset.fallback-error": "Error sending reset email",
    "auth.reset.back": "Back to Login",
    "auth.reset.send": "Send Reset Email",
    "auth.reset.success-title": "Email Sent!",
    "auth.reset.success-subtitle":
      "Check your email inbox and follow the instructions to reset your password.",

    // Sidebar
    "sidebar.new-chat": "New chat",
    "sidebar.ai-lab": "AI Lab",
    "sidebar.editor": "Editor",
    "sidebar.library": "Library",
    "sidebar.developers": "Developers",
    "sidebar.search-chats": "Search chats",
    "sidebar.your-chats": "Your chats",
    "sidebar.my-profile": "My Profile",
    "sidebar.support": "Support",
    "sidebar.support-whatsapp": "WhatsApp Support",
    "sidebar.log-out": "Log Out",
    "sidebar.edit-chat": "Edit Chat",
    "sidebar.chat-name": "Chat name",
    "sidebar.enter-chat-name": "Enter chat name",
    "sidebar.cancel": "Cancel",
    "sidebar.save": "Save",
    "sidebar.saving": "Saving...",
    "sidebar.delete-chat": "Delete Chat",
    "sidebar.delete-confirm": "Are you sure you want to delete this chat? This action cannot be undone.",
    "sidebar.delete": "Delete",
    "sidebar.deleting": "Deleting...",

    // Library
    "library.title": "Media Library",
    "library.all": "All",
    "library.images": "Images",
    "library.videos": "Videos",
    "library.audio": "Audio",
    "library.search": "Search media...",
    "library.empty": "No media found",
    "library.delete": "Delete",
    "library.download": "Download",
    "library.ai-generated": "AI Generated",
    "library.public": "Public",
    "library.private": "Private",
    "library.make-public": "Make Public",
    "library.make-private": "Make Private",
    "library.rename": "Rename",
    "library.rename-title": "Rename File",
    "library.rename-placeholder": "Enter new name",
    "library.save": "Save",
    "library.cancel": "Cancel",
    "library.delete-title": "Delete File",
    "library.delete-confirm": "Are you sure you want to delete this file? This action cannot be undone.",
    "library.loading": "Loading...",
    "library.uploads": "Uploads",
    "library.projects": "Projects",
    "library.send-to-editor": "Send to Editor",
    "library.prev": "Previous",
    "library.next": "Next",

    // Profile
    "profile.title": "My Profile",
    "profile.settings": "Settings",
    "profile.edit": "Edit Profile",
    "profile.save": "Save Changes",
    "profile.cancel": "Cancel",
    "profile.name": "Full Name",
    "profile.email": "Email",
    "profile.location": "Location",
    "profile.password": "Password",
    "profile.confirm-password": "Confirm Password",
    "profile.wallet": "Solana Wallet",
    "profile.referral": "Referral Code",
    "profile.copy": "Copy",
    "profile.copied": "Copied!",
    "profile.upload-photo": "Upload Photo",
    "profile.saving": "Saving...",
    "profile.password-mismatch": "Passwords do not match",
    "profile.password-empty": "Password fields cannot be empty",
    "profile.role.content-creator": "Content Creator",
    "profile.username": "Username",
    "profile.member-since": "Member Since",
    "profile.activity-stats": "Activity Stats",
    "profile.total-projects": "Total Projects",
    "profile.completed": "Completed",
    "profile.not-provided": "Not provided",
    "profile.not-available": "Not available",
    "profile.user-name": "User Name",
    "profile.saving-changes": "Saving changes...",
    "profile.new-image-selected": "New image selected",
    "profile.enter-name": "Enter your name",
    "profile.enter-email": "Enter your email",
    "profile.enter-new-password": "Enter new password (optional)",
    "profile.confirm-new-password": "Confirm new password",

    // My Subscription

    // Chat main
    "chat.placeholder": "Describe what you want to create...",
    "chat.send": "Send",
    "chat.cancel": "Cancel",
    "chat.typing": "ReelBot is thinking...",
    "chat.generating.image": "Generating image…",
    "chat.generating.video": "Generating video…",
    "chat.generating.kling-note": "This may take a while depending on the model's queue times.",
    "chat.generating.failed": "Generation failed.",
    "chat.generating.refunded": "Your tokens were refunded.",
    "chat.generating.timeout": "Still working — it will show up in your notifications and library when ready.",
    "chat.new-chat": "New chat",
    "chat.welcome": "What do you want to create today?",
    "chat.tokens": "tokens",
    "chat.copy": "Copy",
    "chat.copied": "Copied",
    "chat.regenerate": "Regenerate",
    "chat.download": "Download",
    "chat.error": "Something went wrong. Please try again.",
    "chat.attach": "Attach file",
    "chat.image-url": "Image URL",
    "chat.buy-tokens": "Buy Tokens",
    "chat.actions.go-to-editor": "Go to editor",
    "chat.actions.buy-tokens": "Buy tokens",
    "chat.actions.whatsapp-support": "Chat on WhatsApp",
    "chat.stop": "Stop generation",
    "chat.notifications": "Notifications",
    "chat.no-notifications": "No notifications",
    "chat.time.just-now": "less than 1 min ago",
    "chat.time.minutes-ago": "{n} min ago",
    "chat.time.hour-ago": "{n} hr ago",
    "chat.time.hours-ago": "{n} hrs ago",
    "chat.time.day-ago": "{n} day ago",
    "chat.time.days-ago": "{n} days ago",
    "chat.copy-clipboard": "Copy to clipboard",
    "chat.report-tooltip": "Report content",
    "chat.report-action": "Report",
    "chat.report-title": "Report content",
    "chat.report-description": "Help us keep Reelmotion safe. Reports are reviewed by our moderation team and acted on within 24 hours.",
    "chat.report-reason": "Reason",
    "chat.report-reason.sexual": "Sexual or explicit content",
    "chat.report-reason.csam": "Child sexual abuse material (CSAM)",
    "chat.report-reason.violence": "Violence or graphic content",
    "chat.report-reason.hate": "Hate speech or discrimination",
    "chat.report-reason.harassment": "Harassment or bullying",
    "chat.report-reason.self-harm": "Self-harm or suicide",
    "chat.report-reason.illegal": "Illegal activity",
    "chat.report-reason.misinformation": "Misinformation or scam",
    "chat.report-reason.other": "Other",
    "chat.report-details": "Additional details (optional)",
    "chat.report-details-placeholder": "Describe the issue with this content...",
    "chat.report-submit": "Submit report",
    "chat.report-submitting": "Submitting...",
    "chat.report-required": "Please select a reason to continue.",
    "chat.report-success-title": "Report submitted",
    "chat.report-success": "Thank you. Our moderation team will review this content and take appropriate action within 24 hours. Users who repeatedly violate our policies will have their accounts suspended.",
    "chat.report-close": "Close",
    "chat.search-country": "Search country...",
    "chat.welcome": "How can I help you?",
    "chat.quick.create": "Create",
    "chat.quick.edit": "Edit",
    "chat.quick.analyze": "Analyze",
    "chat.quick.how-to-use": "How to use",
    "chat.quick.how-to-use-message":
      "How do I use Reelmotion? Give me a quick, beginner-friendly overview of the platform and what I can do here.",
    "chat.quick.back": "Back",
    "chat.quick.create-image": "Create AI Image",
    "chat.quick.create-video": "Create AI Video",
    "chat.quick.create-project": "Create Project",
    "chat.quick.create-voice": "Create Voice",
    "chat.quick.edit-image": "Edit Image",
    "chat.quick.edit-video": "Edit Video",
    "chat.quick.analyze-image": "Analyze Image",
    "chat.quick.analyze-video": "Analyze Video",
    "chat.tokens.title": "Buy Tokens",
    "chat.tokens.gateway": "Select Payment Gateway",
    "chat.tokens.method": "Payment Method",
    "chat.tokens.success": "Purchase Successful",
    "chat.tokens.error": "Payment Error",
    "chat.tokens.description": "Enter the amount you want to spend. Each dollar gives you 100 tokens. Minimum purchase is $6.",
    "chat.tokens.amount": "Amount (USD)",
    "chat.tokens.subtotal": "Subtotal:",
    "chat.tokens.total": "Total Amount:",
    "chat.tokens.receive": "Tokens you'll receive:",
    "chat.tokens.continue": "Continue to Payment",
    "chat.tokens.total-label": "Total:",
    "chat.tokens.tokens-label": "Tokens:",
    "chat.tokens.choose-method": "Choose Payment Method",
    "chat.tokens.card": "Credit/Debit Card",
    "chat.tokens.card-desc": "Pay directly with your card",
    "chat.tokens.crypto": "Pay with Crypto",
    "chat.tokens.crypto-desc": "USDC on Solana via Phantom",
    "chat.tokens.back": "Back",
    "chat.tokens.continue-btn": "Continue",
    "chat.tokens.card-payment": "Credit/Debit Card Payment",
    "chat.tokens.billing": "Billing Address",
    "chat.tokens.first-name": "First Name",
    "chat.tokens.last-name": "Last Name",
    "chat.tokens.address": "Address",
    "chat.tokens.country": "Country",
    "chat.tokens.postal": "Postal Code",
    "chat.tokens.card-number": "Card Number",
    "chat.tokens.processing": "Processing...",
    "chat.tokens.enter-card": "Enter card details",
    "chat.tokens.pay": "Pay",
    "chat.tokens.pay-usdc": "Pay ${amount} with USDC",
    "chat.tokens.crypto-payment": "Crypto Payment",
    "chat.tokens.phantom-required": "Phantom Wallet Required",
    "chat.tokens.phantom-desc": "You need Phantom wallet to pay with USDC on Solana",
    "chat.tokens.connect-wallet": "Connect Your Wallet",
    "chat.tokens.connecting": "Connecting...",
    "chat.tokens.connect-btn": "Connect Phantom Wallet",
    "chat.tokens.wallet-connected": "Wallet Connected",
    "chat.tokens.disconnect": "Disconnect",
    "chat.tokens.processing-payment": "Processing Payment...",
    "chat.tokens.crypto-unavailable": "Crypto Payments Unavailable",
    "chat.tokens.crypto-unavailable-desc": "Crypto payments are temporarily unavailable.",
    "chat.tokens.success-title": "Purchase Successful!",
    "chat.tokens.success-desc": "tokens have been added to your account.",
    "chat.tokens.new-balance": "New Balance:",
    "chat.tokens.close": "Close",
    "chat.tokens.failed": "Payment Failed",
    "chat.tokens.try-again": "Try Again",

    // AI Lab Modal
    "ailab.title": "AI Lab",
    "ailab.tokens": "tokens",
    "ailab.search-voices": "Search voices...",
    "ailab.filter.language": "Language",
    "ailab.filter.gender": "Gender",
    "ailab.filter.age": "Age",
    "ailab.boost": "Boost",
    "ailab.stability": "Stability",
    "ailab.similarity": "Similarity",
    "ailab.style": "Style",
    "ailab.enter-tts-text": "Enter the text you want to convert to speech...",
    "ailab.generate-voice": "Generate Voice",
    "ailab.generating": "Generating...",
    "ailab.processing": "Processing...",
    "ailab.this-may-take": "This may take a moment",
    "ailab.load-more": "Load more",
    "ailab.loading": "Loading...",
    "ailab.tooltip.image": "Generate Image",
    "ailab.tooltip.video": "Generate Video",
    "ailab.tooltip.voice": "Create Voice",
    "ailab.tooltip.generate": "Generate",
    "ailab.tooltip.preview": "Preview",
    "ailab.tooltip.download": "Download",
    "ailab.tooltip.remove": "Remove",
    "ailab.describe-video": "Describe the video you want to create",
    "ailab.describe-image": "Describe the image you want to create",
    "ailab.negative-prompt": "Negative Prompt",
    "ailab.negative-prompt-placeholder":
      "List what to exclude from your video (e.g. if you don't want trees, type 'trees')",
    "ailab.max-img": "Max {n} img",
    "ailab.max-imgs": "Max {n} imgs",
    "ailab.upload-ref-video": "Upload reference (video/image)",
    "ailab.upload-ref-image": "Upload reference image (max {n})",
    "ailab.drop-images": "Drop images here",
    "ailab.duration-label": "DUR:",
    "ailab.duration-auto": "Auto",
    "ailab.resolution-label": "RES:",
    "ailab.aspect-label": "AR:",
    "ailab.mode-label": "MODE:",
    "ailab.mode-auto": "Auto",
    "ailab.mode-reference": "Reference",
    "ailab.mode-edit": "Edit",
    "ailab.audio": "Audio",
    "ailab.select-model": "Select model",
    "ailab.select-image-model": "Select Image Model",
    "ailab.select-video-model": "Select Video Model",
    "ailab.tokens-used": "{n} tokens used",
    "ailab.tokens-remaining": "{n} remaining",
    "ailab.buy-tokens": "Buy Tokens",
    "ailab.insufficient-tokens": "Insufficient tokens.",
    "ailab.error-default": "An error occurred during generation",
    "ailab.uploaded": "Uploaded",
    "ailab.new": "New",
    "ailab.saved-attachments": "Saved to attachments",
    "ailab.generated-voice": "Generated Voice",
    "ailab.uploading-reference": "Uploading reference media...",
    "ailab.generating-video": "Generating video...",
    "ailab.video-generated": "Video generated successfully",
    "ailab.chars": "chars",
    "ailab.confirm-generation": "Confirm Generation",
    "ailab.confirm-cost-image": "This image generation will use",
    "ailab.confirm-cost-video": "This video generation will use",
    "ailab.confirm-cost-voice": "This voice generation will use",
    "ailab.confirm-cancel": "Cancel",
    "ailab.confirm-btn": "Confirm",
    "ailab.tokens-suffix": "Tokens",
    "ailab.tokens-per-sec": "{n} tokens/sec",
    "ailab.tokens-per-image": "{cost} tokens × {count} images",

    // Help Button menu
    "help.reelbot": "Reelbot",
    "help.reelbot-subtitle": "Support Chat",
    "help.suggestions": "Suggestions",
    "help.suggestions-subtitle": "Help us improve",

    // Reelbot widget
    "reelbot.online": "Online",
    "reelbot.greeting":
      "Hi! I'm Reelbot, your support assistant. How can I help you today?",
    "reelbot.placeholder": "Type your message...",
    "reelbot.error-default":
      "I'm sorry, I couldn't process your request at the moment. Please try again.",
    "reelbot.error-connection":
      "I'm sorry, I'm having trouble connecting right now. Please try again later or contact our support team.",

    // Suggestions form
    "suggestions.title": "Suggestions",
    "suggestions.subtitle": "Help us improve Reelmotion",
    "suggestions.category": "Category",
    "suggestions.select-category": "Select a category",
    "suggestions.cat.ui": "User Interface",
    "suggestions.cat.ai": "AI Generation",
    "suggestions.cat.performance": "Performance",
    "suggestions.cat.features": "New Features",
    "suggestions.cat.documentation": "Documentation",
    "suggestions.cat.support": "Support",
    "suggestions.cat.other": "Other",
    "suggestions.rating": "Overall rating",
    "suggestions.your-suggestion": "Your suggestion",
    "suggestions.placeholder":
      "Tell us what we can improve or what new features you'd like to see...",
    "suggestions.email": "Email",
    "suggestions.email-placeholder": "your@email.com",
    "suggestions.email-help":
      "We need your email to follow up on your suggestion",
    "suggestions.send": "Send Suggestion",
    "suggestions.sending": "Sending...",
    "suggestions.thanks-title": "Thank you for your feedback!",
    "suggestions.thanks-desc":
      "Your suggestion has been sent and will help us improve Reelmotion.",
    "suggestions.alert-fill":
      "Please fill in all required fields with valid information.",
    "suggestions.alert-error":
      "Error sending suggestion. Please try again.",

    // Footer
    "footer.rights": "All rights reserved.",
    "footer.terms": "Terms of Service",
    "footer.privacy": "Privacy Policy",
    "footer.contact": "Contact Us",
    "footer.product": "Product",
    "footer.tagline": "From campaign brief to published content — powered by AI.",
    "footer.copyright": "\u00a9 2026 Reelmotion AI. All rights reserved.",
    "footer.builtWith":
      "Built with React \u00b7 Tailwind CSS \u00b7 Framer Motion \u00b7 Stripe \u00b7 ElevenLabs \u00b7 Pusher \u00b7 Solana",

    // AI Lab — generation status
    "gen.image.pending": "Your image is being generated… we'll let you know when it's ready.",
    "gen.video.pending": "Your video is being generated… we'll let you know when it's ready.",
    "gen.inProgress": "Generation in progress.",
    "gen.failed": "Generation failed. Your tokens were refunded.",
    "gen.timeout":
      "Generation is taking longer than expected. Check your library in a few minutes.",

    // Editor — alerts
    "editor.err.noTimelineItems": "There are no timeline items to pre-render.",
    "editor.err.screenCaptureDenied":
      "You must allow screen capture access to pre-render.",
    "editor.err.preRender": "Error during pre-rendering: ",
    "editor.err.imagesOnly": "Please drop image files only.",
    "editor.err.uploadImages": "Error uploading images. Please try again.",
    "editor.err.audioOnly": "Please drop valid audio files.",
    "editor.err.uploadMusic": "Error uploading music. Please try again.",
    "editor.err.uploadSounds": "Error uploading sounds. Please try again.",
    "editor.err.uploadVoice": "Error uploading voice. Please try again.",
    "editor.err.deleteMusicFailed": "Could not delete the music.",
    "editor.err.deleteMusic": "Error deleting music.",
    "editor.err.deleteVoiceFailed": "Could not delete the voice.",
    "editor.err.deleteVoice": "Error deleting voice.",
    "editor.err.deleteSoundFailed": "Could not delete the sound.",
    "editor.err.deleteSound": "Error deleting sound.",

    // Editor — tooltips
    "editor.tip.deleteMusic": "Delete music",
    "editor.tip.deleteScene": "Delete scene",
    "editor.tip.deleteElement": "Delete element",
    "editor.tip.deleteVoice": "Delete voice",
    "editor.tip.deleteSound": "Delete sound",
    "editor.tip.changeStart": "Change start",
    "editor.tip.changeDuration": "Change duration",

    // Create frame
    "frame.err.promptAndRatio": "Please enter a prompt and select the aspect ratio.",

    // Suggestions
    "suggestions.err.send": "Error sending the suggestion",

    // Onboarding
    "onboarding.title": "Tell us about your business",
    "onboarding.subtitle": "Reelmotion is built for teams and brands. This helps us set up your account.",
    "onboarding.step": "Step",
    "onboarding.of": "of",
    "onboarding.back": "Back",
    "onboarding.next": "Next",
    "onboarding.submit": "Finish",
    "onboarding.submitting": "Saving...",
    "onboarding.err.generic": "We couldn't save your answers. Please try again.",
    "onboarding.err.url": "Enter a valid URL (e.g. https://yourcompany.com)",
    "onboarding.company": "Company or brand name",
    "onboarding.company.ph": "Acme Studios",
    "onboarding.website": "Company website or LinkedIn",
    "onboarding.website.ph": "https://yourcompany.com",
    "onboarding.country": "Country",
    "onboarding.country.ph": "Select your country",
    "onboarding.role": "What's your role?",
    "onboarding.role.marketing": "Marketing / Growth",
    "onboarding.role.agency": "Agency",
    "onboarding.role.founder": "Founder / CEO",
    "onboarding.role.creator": "Content creator",
    "onboarding.role.production": "Production / Video",
    "onboarding.role.other": "Other",
    "onboarding.team_size": "Team size",
    "onboarding.team_size.1": "Just me",
    "onboarding.team_size.2-10": "2-10",
    "onboarding.team_size.11-50": "11-50",
    "onboarding.team_size.51-200": "51-200",
    "onboarding.team_size.200+": "200+",
    "onboarding.use_case": "What will you use Reelmotion for?",
    "onboarding.use_case.ads": "Paid ads",
    "onboarding.use_case.social": "My brand's social media",
    "onboarding.use_case.product": "Product / e-commerce videos",
    "onboarding.use_case.agency_clients": "Content for agency clients",
    "onboarding.use_case.personal": "Personal use",
    "onboarding.monthly_volume": "How many videos do you produce per month?",
    "onboarding.monthly_volume.<5": "Fewer than 5",
    "onboarding.monthly_volume.5-20": "5-20",
    "onboarding.monthly_volume.20-100": "20-100",
    "onboarding.monthly_volume.100+": "100+",
    "onboarding.budget": "Monthly budget for video content",
    "onboarding.budget.0": "$0",
    "onboarding.budget.<100": "Under $100",
    "onboarding.budget.100-500": "$100 - $500",
    "onboarding.budget.500-2k": "$500 - $2,000",
    "onboarding.budget.2k+": "$2,000+",
    "onboarding.source": "How did you hear about us?",
    "onboarding.source.google": "Google",
    "onboarding.source.social": "Social media",
    "onboarding.source.referral": "Referral",
    "onboarding.source.product_hunt": "Product Hunt",
    "onboarding.source.other": "Other",
    "onboarding.done.granted.title": "You're all set!",
    "onboarding.done.granted.body": "We've credited 20 welcome tokens so you can start creating.",
    "onboarding.done.granted.cta": "Start creating",
    "onboarding.done.title": "Your account is ready",
    "onboarding.done.body": "Generating images and videos requires tokens. Buy a pack and start today.",
    "onboarding.done.buy": "Buy tokens",
    "onboarding.done.explore": "Explore the app",
  },

  es: {
    // Navbar
    "nav.features": "Funciones",
    "nav.ai-agent": "Agente IA",
    "nav.editor": "Editor",
    "nav.pricing": "Precios",
    "nav.integrations": "Integraciones",
    "nav.developers": "Developers",
    "nav.teams": "Equipos",
    "nav.get-started": "Comenzar",
    "nav.go-to-dashboard": "Ir al Panel",

    // Hero
    "hero.badge": "Kling V3 · Seedance 2.5 · Veo 3.1 — Ya disponibles",
    "hero.title": "Lanza tu próxima campaña de anuncios — antes de la comida.",
    "hero.subtitle":
      "Da el brief como a un equipo creativo. Reelmotion AI escribe el prompt, elige el modelo y produce video, imágenes y voz con calidad de estudio para tus campañas en minutos — una variación o cincuenta. Paga solo lo que generas. Sin suscripción, nunca.",
    "hero.cta-primary": "Empieza tu campaña",
    "hero.cta-secondary": "Míralo en acción",
    "hero.note": "Sin suscripciones · Sin asientos · Pago por consumo — 20 tokens gratis para empezar",
    "hero.go-to-dashboard": "Ir al Panel",

    // Social Proof
    "social-proof.assets-generated": "Assets de campaña generados",
    "social-proof.ai-models": "Modelos de IA, una plataforma",
    "social-proof.hours-value": "Horas",
    "social-proof.hours-label": "No semanas",
    "social-proof.countries": "Países atendidos",
    "social-proof.variations": "Variaciones por brief",

    // Features
    "features.title":
      "Todo tu equipo puede publicar video — sin cuello de botella creativo",
    "features.subtitle":
      "La mayoría de las herramientas de video con IA asumen que tienes un prompt engineer en el equipo. Reelmotion AI no. Nuestro agente convierte el brief de cualquier miembro del equipo en un prompt de nivel producción y entrega assets que parecen hechos por agencia — para que las campañas no se atoren esperando a un especialista, ni se atoren por volumen. Un video hero o veinte variaciones de anuncio: el proceso es igual de rápido.",
    "features.video.title": "Convierte un brief en un set completo de videos",
    "features.video.description":
      "Describe el spot, anuncio o clip social que necesitas. Nuestro agente escribe el prompt y te deja elegir entre los mejores modelos — Seedance 2.5, Veo 3.1, Kling V3, Runway 4.5 y más. Genera varias versiones o formatos en paralelo, listos para probar en cada canal.",
    "features.image.title": "Visuales on-brand para cada canal",
    "features.image.description":
      "Escribe cómo debe verse el creativo. Nuestra IA lo convierte en un prompt preciso y genera imágenes 4K con Nano Banana 2, GPT Image 2 o Seedream 5.0.",
    "features.audio.title": "Voz en off lista para emitir, sin reservar estudio",
    "features.audio.description":
      "Escribe el guion, elige una voz, genera. El TTS multilenguaje de ElevenLabs entrega narración con calidad de agencia para cada mercado al que vendes.",

    // ReelBot
    "reelbot.title": "Tu equipo creativo bajo demanda",
    "reelbot.subtitle":
      "Sin prompt engineering, sin idas y vueltas del brief. Nuestra IA hace las preguntas correctas, construye el prompt perfecto y genera resultados listos para publicar — para que tu equipo pase de concepto a campaña el mismo día.",
    "reelbot.feat.imageGen": "Generaci\u00f3n de Im\u00e1genes",
    "reelbot.feat.imageGenDesc":
      "Texto a imagen e imagen a imagen con Nano Banana 2, GPT Image 2 y Seedream",
    "reelbot.feat.videoGen": "Generaci\u00f3n de Video",
    "reelbot.feat.videoGenDesc":
      "10 modelos de video: Seedance 2.5, Veo 3.1, Runway 4.5, Kling V3 y m\u00e1s",
    "reelbot.feat.tts": "Texto a Voz",
    "reelbot.feat.ttsDesc": "Voces multiling\u00fces con ElevenLabs",
    "reelbot.feat.vision": "An\u00e1lisis Visual",
    "reelbot.feat.visionDesc":
      "Sube una imagen o video y ReelBot lo entiende al instante — sin descripciones manuales",
    "reelbot.feat.prompts": "Constructor de Prompts Guiado",
    "reelbot.feat.promptsDesc":
      "ReelBot pregunta paso a paso para construir el prompt exacto que clava el resultado que quieres — cero adivinanzas",
    "reelbot.step1.title": "Da el brief de tu campaña con tus palabras",
    "reelbot.step1.desc":
      "Sin conocimientos de prompts. Solo dile a ReelBot lo que est\u00e1s imaginando.",
    "reelbot.step2.title": "ReelBot crea tu prompt",
    "reelbot.step2.desc":
      "El agente hace preguntas inteligentes de seguimiento y escribe un prompt de nivel profesional por ti.",
    "reelbot.step3.title": "Elige modelo y ajustes",
    "reelbot.step3.desc":
      "Selecciona modelo de IA, resoluci\u00f3n, relaci\u00f3n de aspecto y duraci\u00f3n — todo en un solo lugar.",
    "reelbot.step4.title": "Ve el costo antes de comprometer presupuesto",
    "reelbot.step4.desc":
      "Transparencia total — el costo exacto en tokens antes de generar, sin sorpresas.",
    "reelbot.step5.title": "Genera y entrega",
    "reelbot.step5.desc":
      "Tu contenido está listo en segundos — descárgalo o mándalo directo al editor para el acabado final.",

    // Chat Demo
    "demo.title": "Sin prompts. Sin curva de aprendizaje. Solo resultados.",
    "demo.subtitle":
      "Habla con ReelBot como hablar\u00edas con un colega — \u00e9l se encarga de la ingenier\u00eda de prompts para que obtengas contenido profesional desde el primer intento.",
    "demo.caption":
      "De idea en bruto a contenido pulido — mira a ReelBot trabajar",
    "demo.showcaseTitle": "Creado con Reelmotion AI",
    "demo.showcaseAlt": "Contenido generado con IA",

    // Editor
    "editor.title": "Del primer borrador a listo para el cliente — todo en una pestaña.",
    "editor.subtitle":
      "Un editor de video profesional integrado en la misma plataforma donde generas el contenido. Sin exportar a otra herramienta, sin cambiar de app entre tu equipo y freelancers.",
    "editor.feat.timeline": "Timeline Multipista",
    "editor.feat.timelineDesc":
      "Arrastra, suelta y haz zoom frame a frame — edici\u00f3n de precisi\u00f3n al alcance de tu mano",
    "editor.feat.captions": "Texto y Subt\u00edtulos",
    "editor.feat.captionsDesc":
      "Textos animados, subt\u00edtulos y m\u00e1s de 9 estilos de overlay listos para usar",
    "editor.feat.overlays": "Capas de Imagen y Video",
    "editor.feat.overlaysDesc":
      "Apila clips, im\u00e1genes, formas, stickers y subt\u00edtulos en pistas separadas",
    "editor.feat.audio": "Pistas de Audio",
    "editor.feat.audioDesc":
      "Agrega m\u00fasica, locuciones y efectos de sonido con control de volumen independiente",
    "editor.feat.stock": "Stock Ilimitado",
    "editor.feat.stockDesc":
      "Millones de im\u00e1genes y videos libres de derechos v\u00eda Pexels — integrado directamente",
    "editor.feat.cloud": "Renderizado en la Nube",
    "editor.feat.cloudDesc":
      "Exporta en alta calidad en la nube — tu computadora no hace nada pesado",
    "editor.feat.autosave": "Autoguardado y Proyectos",
    "editor.feat.autosaveDesc":
      "Cada cambio se guarda autom\u00e1ticamente. Retoma cualquier proyecto justo donde lo dejaste.",
    "editor.feat.browser": "Sin Instalaci\u00f3n",
    "editor.feat.browserDesc":
      "Funciona en cualquier dispositivo y navegador — abre una pesta\u00f1a y empieza a editar",

    // Pricing
    "pricing.title": "Sin suscripciones. Paga solo lo que usas.",
    "pricing.subtitle":
      "Sin planes mensuales, sin asientos, sin ataduras. Compra tokens cuando los necesites y úsalos en cualquier modelo — ves el costo exacto de cada generación antes de generar.",
    "pricing.payg.name": "Pago por consumo",
    "pricing.payg.badge": "Sin suscripción",
    "pricing.payg.rate": "1 token = US$0.01 · mismo precio para cualquier tamaño de equipo",
    "pricing.payg.min": "Recarga desde $6 · sin gasto mínimo mensual",
    "pricing.payg.cta": "Empieza gratis",
    "pricing.payg.free": "20 tokens gratis al registrarte · sin tarjeta",
    "pricing.payg.included": "Todo incluido",
    "pricing.includes.video": "Generación de video",
    "pricing.includes.image": "Generación de imágenes",
    "pricing.includes.voice": "Voz en off y TTS",
    "pricing.includes.models": "Los 15+ modelos de IA",
    "pricing.includes.editor": "Editor de video en la nube",
    "pricing.includes.library": "Librería de medios compartida",
    "pricing.includes.agent": "AI Agent (ReelBot)",
    "pricing.includes.api": "API REST y servidor MCP",
    "pricing.example.video": "Video de 5s · Kling V3 720p",
    "pricing.example.image": "1 imagen · Nano Banana 2",
    "pricing.example.voice": "Voz en off de 1,000 caracteres · ElevenLabs",
    "pricing.example.note":
      "Precios reales de la app. Las generaciones fallidas se reembolsan automáticamente.",
    "pricing.enterprise.title": "¿Campañas a gran escala?",
    "pricing.enterprise.subtitle":
      "Volumen de tokens a medida y soporte de integración para agencias y marcas que producen cientos de assets al mes.",
    "pricing.enterprise.bullet1": "Volumen de tokens a medida",
    "pricing.enterprise.bullet2": "Soporte de integración API y MCP",
    "pricing.enterprise.bullet3": "Línea directa con el equipo",
    "pricing.enterprise.cta": "Hablar con ventas",

    // CTA Banner
    "cta.title": "Tu próxima campaña está a un brief de distancia.",
    "cta.subtitle":
      "Únete a equipos de marketing y agencias que publican video en volumen — rápido, sin el costo de producción y sin suscripción.",
    "cta.button": "Empieza tu campaña",
    "cta.note": "Sin tarjeta. Sin suscripción. 20 tokens gratis — después pagas solo lo que usas.",

    // Integrations
    "integrations.badge": "✦ Integraciones",
    "integrations.title": "Conecta Reelmotion a Claude, ChatGPT o tu propio agente de IA",
    "integrations.subtitle":
      "Reelmotion incluye un servidor MCP remoto y una API REST. Conéctalo una vez con OAuth y deja que tu agente dé el brief, genere y entregue los assets de campaña — sin SDK, sin código pegamento.",
    "integrations.logos-caption": "Funciona con",
    "integrations.any-agent": "Cualquier cliente MCP",
    "integrations.step1.title": "Conéctalo en un clic",
    "integrations.step1.desc":
      "Agrega Reelmotion como conector en Claude, ChatGPT, Claude Code o Cursor. OAuth 2.1 con PKCE se encarga del acceso — o usa una API key.",
    "integrations.step2.title": "Da el brief desde tu agente",
    "integrations.step2.desc":
      "Pide \"20 cortes verticales para TikTok\" — tu agente llama a create_video, create_image y create_audio con el modelo y ajustes correctos.",
    "integrations.step3.title": "Los assets llegan a tu librería",
    "integrations.step3.desc":
      "Cada generación aparece en tu librería y editor de Reelmotion, lista para pulir y publicar.",
    "integrations.cta": "Lee la documentación para developers",
    "integrations.note": "MCP · OAuth 2.1 + PKCE · REST API · 1 token = US$0.01",

    // Teams
    "teams.badge": "✦ Hecho para equipos",
    "teams.title": "Un solo espacio para toda tu campaña",
    "teams.subtitle":
      "Mantén cada asset en un solo lugar y deja que tu equipo y tus agentes trabajen desde la misma librería.",
    "teams.folders.title": "Carpetas de proyecto compartidas",
    "teams.folders.desc":
      "Comparte una carpeta con tu equipo para que todos trabajen desde los mismos assets de campaña — sin reenviar archivos.",
    "teams.library.title": "Librería de medios compartida",
    "teams.library.desc":
      "Cada imagen, video y voz generada en una carpeta compartida queda disponible para todos sus miembros, en la librería y en el editor.",
    "teams.pipeline.title": "Pipeline listo para agentes",
    "teams.pipeline.desc":
      "Conecta Reelmotion a tus propios agentes y flujos vía MCP o la API REST y genera assets de campaña a escala.",

    // Auth Modal
    "auth.tab.signin": "Iniciar Sesi\u00f3n",
    "auth.tab.register": "Registrarse",
    "auth.signin.title": "Bienvenido de vuelta",
    "auth.signin.subtitle": "Ingresa tus credenciales para acceder a tu cuenta",
    "auth.signin.email-placeholder": "Email o nombre de usuario",
    "auth.signin.password-placeholder": "Contrase\u00f1a",
    "auth.signin.forgot-password": "\u00bfOlvidaste tu contrase\u00f1a?",
    "auth.signin.button": "Iniciar Sesi\u00f3n",
    "auth.signin.email-required": "El email es obligatorio",
    "auth.signin.password-required": "La contrase\u00f1a es obligatoria",
    "auth.signin.invalid-credentials":
      "El email o nombre de usuario es incorrecto",
    "auth.register.title": "Crear Cuenta",
    "auth.register.subtitle":
      "\u00danete a la nueva era de la creaci\u00f3n de contenido",
    "auth.register.username-placeholder": "Nombre de usuario",
    "auth.register.name-placeholder": "Nombre completo",
    "auth.register.email-placeholder": "Correo electr\u00f3nico",
    "auth.register.password-placeholder": "Contrase\u00f1a",
    "auth.register.confirm-password-placeholder": "Confirmar contrase\u00f1a",
    "auth.register.accept-terms-prefix": "Acepto los",
    "auth.register.terms": "T\u00e9rminos y Condiciones",
    "auth.register.and": "y la",
    "auth.register.privacy": "Pol\u00edtica de Privacidad",
    "auth.register.accept-terms-error":
      "Debes aceptar la Pol\u00edtica de Privacidad y los T\u00e9rminos y Condiciones para crear una cuenta",
    "auth.register.email-invalid":
      "Por favor ingresa un correo electr\u00f3nico v\u00e1lido",
    "auth.register.password-empty":
      "Los campos de contrase\u00f1a no pueden estar vac\u00edos",
    "auth.register.password-mismatch": "Las contrase\u00f1as no coinciden",
    "auth.register.button": "Crear Cuenta",
    "auth.reset.title": "Restablecer Contrase\u00f1a",
    "auth.reset.subtitle":
      "Ingresa tu correo electr\u00f3nico y te enviaremos instrucciones para restablecer tu contrase\u00f1a.",
    "auth.reset.email-label": "Correo electr\u00f3nico",
    "auth.reset.email-placeholder": "Ingresa tu correo",
    "auth.reset.email-required": "El email es obligatorio",
    "auth.reset.email-invalid":
      "Por favor ingresa un correo electr\u00f3nico v\u00e1lido",
    "auth.reset.network-error":
      "Error de red. Por favor int\u00e9ntalo de nuevo.",
    "auth.reset.fallback-success":
      "Las instrucciones para restablecer tu contrase\u00f1a han sido enviadas a tu correo.",
    "auth.reset.fallback-error":
      "Error al enviar el correo de restablecimiento",
    "auth.reset.back": "Volver al inicio de sesi\u00f3n",
    "auth.reset.send": "Enviar correo",
    "auth.reset.success-title": "Correo enviado",
    "auth.reset.success-subtitle":
      "Revisa tu bandeja de entrada y sigue las instrucciones para restablecer tu contrase\u00f1a.",

    // Sidebar
    "sidebar.new-chat": "Nuevo chat",
    "sidebar.ai-lab": "AI Lab",
    "sidebar.editor": "Editor",
    "sidebar.library": "Biblioteca",
    "sidebar.developers": "Developers",
    "sidebar.search-chats": "Buscar chats",
    "sidebar.your-chats": "Tus chats",
    "sidebar.my-profile": "Mi Perfil",
    "sidebar.support": "Soporte",
    "sidebar.support-whatsapp": "Soporte por WhatsApp",
    "sidebar.log-out": "Cerrar Sesión",
    "sidebar.edit-chat": "Editar Chat",
    "sidebar.chat-name": "Nombre del chat",
    "sidebar.enter-chat-name": "Ingresa el nombre del chat",
    "sidebar.cancel": "Cancelar",
    "sidebar.save": "Guardar",
    "sidebar.saving": "Guardando...",
    "sidebar.delete-chat": "Eliminar Chat",
    "sidebar.delete-confirm": "¿Estás seguro de que quieres eliminar este chat? Esta acción no se puede deshacer.",
    "sidebar.delete": "Eliminar",
    "sidebar.deleting": "Eliminando...",

    // Library
    "library.title": "Biblioteca de Medios",
    "library.all": "Todo",
    "library.images": "Imágenes",
    "library.videos": "Videos",
    "library.audio": "Audio",
    "library.search": "Buscar medios...",
    "library.empty": "No se encontraron medios",
    "library.delete": "Eliminar",
    "library.download": "Descargar",
    "library.ai-generated": "Generado con IA",
    "library.public": "Público",
    "library.private": "Privado",
    "library.make-public": "Hacer Público",
    "library.make-private": "Hacer Privado",
    "library.rename": "Renombrar",
    "library.rename-title": "Renombrar Archivo",
    "library.rename-placeholder": "Ingresa el nuevo nombre",
    "library.save": "Guardar",
    "library.cancel": "Cancelar",
    "library.delete-title": "Eliminar Archivo",
    "library.delete-confirm": "¿Estás seguro de que quieres eliminar este archivo? Esta acción no se puede deshacer.",
    "library.loading": "Cargando...",
    "library.uploads": "Subidas",
    "library.projects": "Proyectos",
    "library.send-to-editor": "Enviar al Editor",
    "library.prev": "Anterior",
    "library.next": "Siguiente",

    // Profile
    "profile.title": "Mi Perfil",
    "profile.settings": "Configuración",
    "profile.edit": "Editar Perfil",
    "profile.save": "Guardar Cambios",
    "profile.cancel": "Cancelar",
    "profile.name": "Nombre Completo",
    "profile.email": "Correo Electrónico",
    "profile.location": "Ubicación",
    "profile.password": "Contraseña",
    "profile.confirm-password": "Confirmar Contraseña",
    "profile.wallet": "Wallet Solana",
    "profile.referral": "Código de Referido",
    "profile.copy": "Copiar",
    "profile.copied": "¡Copiado!",
    "profile.upload-photo": "Subir Foto",
    "profile.saving": "Guardando...",
    "profile.password-mismatch": "Las contraseñas no coinciden",
    "profile.password-empty": "Los campos de contraseña no pueden estar vacíos",
    "profile.role.content-creator": "Creador de Contenido",
    "profile.username": "Usuario",
    "profile.member-since": "Miembro Desde",
    "profile.activity-stats": "Estadísticas de Actividad",
    "profile.total-projects": "Proyectos Totales",
    "profile.completed": "Completados",
    "profile.not-provided": "No proporcionado",
    "profile.not-available": "No disponible",
    "profile.user-name": "Nombre de Usuario",
    "profile.saving-changes": "Guardando cambios...",
    "profile.new-image-selected": "Nueva imagen seleccionada",
    "profile.enter-name": "Ingresa tu nombre",
    "profile.enter-email": "Ingresa tu correo electrónico",
    "profile.enter-new-password": "Ingresa nueva contraseña (opcional)",
    "profile.confirm-new-password": "Confirma la nueva contraseña",

    // My Subscription

    // Chat main
    "chat.placeholder": "Describe lo que quieres crear...",
    "chat.generating.image": "Generando imagen…",
    "chat.generating.video": "Generando video…",
    "chat.generating.kling-note": "Puede tardar dependiendo de los tiempos de espera del modelo.",
    "chat.generating.failed": "La generación falló.",
    "chat.generating.refunded": "Tus tokens fueron reembolsados.",
    "chat.generating.timeout": "Sigue en proceso — aparecerá en tus notificaciones y biblioteca cuando esté lista.",
    "chat.send": "Enviar",
    "chat.cancel": "Cancelar",
    "chat.typing": "ReelBot está pensando...",
    "chat.new-chat": "Nuevo chat",
    "chat.welcome": "¿Qué quieres crear hoy?",
    "chat.tokens": "tokens",
    "chat.copy": "Copiar",
    "chat.copied": "Copiado",
    "chat.regenerate": "Regenerar",
    "chat.download": "Descargar",
    "chat.error": "Algo salió mal. Por favor intenta de nuevo.",
    "chat.attach": "Adjuntar archivo",
    "chat.image-url": "URL de imagen",
    "chat.buy-tokens": "Comprar Tokens",
    "chat.actions.go-to-editor": "Ir al editor",
    "chat.actions.buy-tokens": "Comprar tokens",
    "chat.actions.whatsapp-support": "Escribir por WhatsApp",
    "chat.stop": "Detener generación",
    "chat.notifications": "Notificaciones",
    "chat.no-notifications": "Sin notificaciones",
    "chat.time.just-now": "hace menos de 1 min",
    "chat.time.minutes-ago": "hace {n} min",
    "chat.time.hour-ago": "hace {n} hr",
    "chat.time.hours-ago": "hace {n} hrs",
    "chat.time.day-ago": "hace {n} día",
    "chat.time.days-ago": "hace {n} días",
    "chat.copy-clipboard": "Copiar al portapapeles",
    "chat.report-tooltip": "Denunciar contenido",
    "chat.report-action": "Reportar",
    "chat.report-title": "Denunciar contenido",
    "chat.report-description": "Ayúdanos a mantener Reelmotion seguro. Las denuncias son revisadas por nuestro equipo de moderación y se actúa en menos de 24 horas.",
    "chat.report-reason": "Motivo",
    "chat.report-reason.sexual": "Contenido sexual o explícito",
    "chat.report-reason.csam": "Material de abuso sexual infantil (CSAM)",
    "chat.report-reason.violence": "Violencia o contenido gráfico",
    "chat.report-reason.hate": "Discurso de odio o discriminación",
    "chat.report-reason.harassment": "Acoso o intimidación",
    "chat.report-reason.self-harm": "Autolesiones o suicidio",
    "chat.report-reason.illegal": "Actividad ilegal",
    "chat.report-reason.misinformation": "Desinformación o estafa",
    "chat.report-reason.other": "Otro",
    "chat.report-details": "Detalles adicionales (opcional)",
    "chat.report-details-placeholder": "Describe el problema con este contenido...",
    "chat.report-submit": "Enviar denuncia",
    "chat.report-submitting": "Enviando...",
    "chat.report-required": "Por favor selecciona un motivo para continuar.",
    "chat.report-success-title": "Denuncia enviada",
    "chat.report-success": "Gracias. Nuestro equipo de moderación revisará este contenido y tomará las medidas apropiadas en menos de 24 horas. Los usuarios que infrinjan repetidamente nuestras políticas verán sus cuentas suspendidas.",
    "chat.report-close": "Cerrar",
    "chat.search-country": "Buscar país...",
    "chat.welcome": "¿En qué puedo ayudarte?",
    "chat.quick.create": "Crear",
    "chat.quick.edit": "Editar",
    "chat.quick.analyze": "Analizar",
    "chat.quick.how-to-use": "Cómo usar",
    "chat.quick.how-to-use-message":
      "¿Cómo uso Reelmotion? Dame un resumen rápido y fácil de la plataforma y de lo que puedo hacer aquí.",
    "chat.quick.back": "Volver",
    "chat.quick.create-image": "Crear Imagen con IA",
    "chat.quick.create-video": "Crear Video con IA",
    "chat.quick.create-project": "Crear Proyecto",
    "chat.quick.create-voice": "Crear Voz",
    "chat.quick.edit-image": "Editar Imagen",
    "chat.quick.edit-video": "Editar Video",
    "chat.quick.analyze-image": "Analizar Imagen",
    "chat.quick.analyze-video": "Analizar Video",
    "chat.tokens.title": "Comprar Tokens",
    "chat.tokens.gateway": "Seleccionar Pasarela de Pago",
    "chat.tokens.method": "Método de Pago",
    "chat.tokens.success": "Compra Exitosa",
    "chat.tokens.error": "Error de Pago",
    "chat.tokens.description": "Ingresa el monto que quieres gastar. Cada dólar te da 100 tokens. La compra mínima es de $6.",
    "chat.tokens.amount": "Monto (USD)",
    "chat.tokens.subtotal": "Subtotal:",
    "chat.tokens.total": "Monto Total:",
    "chat.tokens.receive": "Tokens que recibirás:",
    "chat.tokens.continue": "Continuar al Pago",
    "chat.tokens.total-label": "Total:",
    "chat.tokens.tokens-label": "Tokens:",
    "chat.tokens.choose-method": "Elige el Método de Pago",
    "chat.tokens.card": "Tarjeta de Crédito/Débito",
    "chat.tokens.card-desc": "Paga directamente con tu tarjeta",
    "chat.tokens.crypto": "Pagar con Cripto",
    "chat.tokens.crypto-desc": "USDC en Solana vía Phantom",
    "chat.tokens.back": "Volver",
    "chat.tokens.continue-btn": "Continuar",
    "chat.tokens.card-payment": "Pago con Tarjeta",
    "chat.tokens.billing": "Dirección de Facturación",
    "chat.tokens.first-name": "Nombre",
    "chat.tokens.last-name": "Apellido",
    "chat.tokens.address": "Dirección",
    "chat.tokens.country": "País",
    "chat.tokens.postal": "Código Postal",
    "chat.tokens.card-number": "Número de Tarjeta",
    "chat.tokens.processing": "Procesando...",
    "chat.tokens.enter-card": "Ingresa los datos de tu tarjeta",
    "chat.tokens.pay": "Pagar",
    "chat.tokens.pay-usdc": "Pagar ${amount} con USDC",
    "chat.tokens.crypto-payment": "Pago con Cripto",
    "chat.tokens.phantom-required": "Se Requiere Phantom Wallet",
    "chat.tokens.phantom-desc": "Necesitas Phantom wallet para pagar con USDC en Solana",
    "chat.tokens.connect-wallet": "Conecta tu Wallet",
    "chat.tokens.connecting": "Conectando...",
    "chat.tokens.connect-btn": "Conectar Phantom Wallet",
    "chat.tokens.wallet-connected": "Wallet Conectada",
    "chat.tokens.disconnect": "Desconectar",
    "chat.tokens.processing-payment": "Procesando Pago...",
    "chat.tokens.crypto-unavailable": "Pagos Cripto No Disponibles",
    "chat.tokens.crypto-unavailable-desc": "Los pagos con cripto están temporalmente no disponibles.",
    "chat.tokens.success-title": "¡Compra Exitosa!",
    "chat.tokens.success-desc": "tokens han sido agregados a tu cuenta.",
    "chat.tokens.new-balance": "Nuevo Saldo:",
    "chat.tokens.close": "Cerrar",
    "chat.tokens.failed": "Pago Fallido",
    "chat.tokens.try-again": "Intentar de Nuevo",

    // AI Lab Modal
    "ailab.title": "AI Lab",
    "ailab.tokens": "tokens",
    "ailab.search-voices": "Buscar voces...",
    "ailab.filter.language": "Idioma",
    "ailab.filter.gender": "Género",
    "ailab.filter.age": "Edad",
    "ailab.boost": "Potenciar",
    "ailab.stability": "Estabilidad",
    "ailab.similarity": "Similitud",
    "ailab.style": "Estilo",
    "ailab.enter-tts-text": "Ingresa el texto que quieres convertir en voz...",
    "ailab.generate-voice": "Generar Voz",
    "ailab.generating": "Generando...",
    "ailab.processing": "Procesando...",
    "ailab.this-may-take": "Esto puede tardar un momento",
    "ailab.load-more": "Cargar más",
    "ailab.loading": "Cargando...",
    "ailab.tooltip.image": "Generar Imagen",
    "ailab.tooltip.video": "Generar Video",
    "ailab.tooltip.voice": "Crear Voz",
    "ailab.tooltip.generate": "Generar",
    "ailab.tooltip.preview": "Vista previa",
    "ailab.tooltip.download": "Descargar",
    "ailab.tooltip.remove": "Eliminar",
    "ailab.describe-video": "Describe el video que quieres crear",
    "ailab.describe-image": "Describe la imagen que quieres crear",
    "ailab.negative-prompt": "Prompt Negativo",
    "ailab.negative-prompt-placeholder":
      "Indica qué excluir de tu video (ej. si no quieres árboles, escribe 'árboles')",
    "ailab.max-img": "Máx {n} img",
    "ailab.max-imgs": "Máx {n} imgs",
    "ailab.upload-ref-video": "Subir referencia (video/imagen)",
    "ailab.upload-ref-image": "Subir imagen de referencia (máx {n})",
    "ailab.drop-images": "Suelta las imágenes aquí",
    "ailab.duration-label": "DUR:",
    "ailab.duration-auto": "Auto",
    "ailab.resolution-label": "RES:",
    "ailab.aspect-label": "AR:",
    "ailab.mode-label": "MODO:",
    "ailab.mode-auto": "Auto",
    "ailab.mode-reference": "Referencia",
    "ailab.mode-edit": "Edición",
    "ailab.audio": "Audio",
    "ailab.select-model": "Seleccionar modelo",
    "ailab.select-image-model": "Seleccionar Modelo de Imagen",
    "ailab.select-video-model": "Seleccionar Modelo de Video",
    "ailab.tokens-used": "{n} tokens usados",
    "ailab.tokens-remaining": "{n} restantes",
    "ailab.buy-tokens": "Comprar Tokens",
    "ailab.insufficient-tokens": "Tokens insuficientes.",
    "ailab.error-default": "Ocurrió un error durante la generación",
    "ailab.uploaded": "Subido",
    "ailab.new": "Nuevo",
    "ailab.saved-attachments": "Guardado en adjuntos",
    "ailab.generated-voice": "Voz Generada",
    "ailab.uploading-reference": "Subiendo medio de referencia...",
    "ailab.generating-video": "Generando video...",
    "ailab.video-generated": "Video generado correctamente",
    "ailab.chars": "caracteres",
    "ailab.confirm-generation": "Confirmar Generación",
    "ailab.confirm-cost-image": "Esta generación de imagen usará",
    "ailab.confirm-cost-video": "Esta generación de video usará",
    "ailab.confirm-cost-voice": "Esta generación de voz usará",
    "ailab.confirm-cancel": "Cancelar",
    "ailab.confirm-btn": "Confirmar",
    "ailab.tokens-suffix": "Tokens",
    "ailab.tokens-per-sec": "{n} tokens/seg",
    "ailab.tokens-per-image": "{cost} tokens × {count} imágenes",

    // Help Button menu
    "help.reelbot": "Reelbot",
    "help.reelbot-subtitle": "Chat de Soporte",
    "help.suggestions": "Sugerencias",
    "help.suggestions-subtitle": "Ayúdanos a mejorar",

    // Reelbot widget
    "reelbot.online": "En línea",
    "reelbot.greeting":
      "¡Hola! Soy Reelbot, tu asistente de soporte. ¿En qué puedo ayudarte hoy?",
    "reelbot.placeholder": "Escribe tu mensaje...",
    "reelbot.error-default":
      "Lo siento, no pude procesar tu solicitud en este momento. Por favor inténtalo de nuevo.",
    "reelbot.error-connection":
      "Lo siento, estoy teniendo problemas de conexión en este momento. Por favor inténtalo de nuevo más tarde o contacta a nuestro equipo de soporte.",

    // Suggestions form
    "suggestions.title": "Sugerencias",
    "suggestions.subtitle": "Ayúdanos a mejorar Reelmotion",
    "suggestions.category": "Categoría",
    "suggestions.select-category": "Selecciona una categoría",
    "suggestions.cat.ui": "Interfaz de Usuario",
    "suggestions.cat.ai": "Generación IA",
    "suggestions.cat.performance": "Rendimiento",
    "suggestions.cat.features": "Nuevas Funciones",
    "suggestions.cat.documentation": "Documentación",
    "suggestions.cat.support": "Soporte",
    "suggestions.cat.other": "Otro",
    "suggestions.rating": "Calificación general",
    "suggestions.your-suggestion": "Tu sugerencia",
    "suggestions.placeholder":
      "Cuéntanos qué podemos mejorar o qué nuevas funciones te gustaría ver...",
    "suggestions.email": "Correo electrónico",
    "suggestions.email-placeholder": "tu@email.com",
    "suggestions.email-help":
      "Necesitamos tu correo para dar seguimiento a tu sugerencia",
    "suggestions.send": "Enviar Sugerencia",
    "suggestions.sending": "Enviando...",
    "suggestions.thanks-title": "¡Gracias por tu opinión!",
    "suggestions.thanks-desc":
      "Tu sugerencia ha sido enviada y nos ayudará a mejorar Reelmotion.",
    "suggestions.alert-fill":
      "Por favor completa todos los campos requeridos con información válida.",
    "suggestions.alert-error":
      "Error al enviar la sugerencia. Por favor inténtalo de nuevo.",

    // Footer
    "footer.rights": "Todos los derechos reservados.",
    "footer.terms": "T\u00e9rminos de Servicio",
    "footer.privacy": "Pol\u00edtica de Privacidad",
    "footer.contact": "Cont\u00e1ctanos",
    "footer.product": "Producto",
    "footer.tagline": "Del brief de campaña al contenido publicado — impulsado por IA.",
    "footer.copyright":
      "\u00a9 2026 Reelmotion AI. Todos los derechos reservados.",
    "footer.builtWith":
      "Hecho con React \u00b7 Tailwind CSS \u00b7 Framer Motion \u00b7 Stripe \u00b7 ElevenLabs \u00b7 Pusher \u00b7 Solana",

    // AI Lab — estado de generación
    "gen.image.pending": "Tu imagen se está generando… te avisaremos cuando esté lista.",
    "gen.video.pending": "Tu video se está generando… te avisaremos cuando esté listo.",
    "gen.inProgress": "Generación en curso.",
    "gen.failed": "La generación falló. Tus tokens fueron reembolsados.",
    "gen.timeout":
      "La generación está tardando más de lo esperado. Revisa tu biblioteca en unos minutos.",

    // Editor — alertas
    "editor.err.noTimelineItems": "No hay elementos en el timeline para pre-renderizar.",
    "editor.err.screenCaptureDenied":
      "Debes permitir el acceso a la captura de pantalla para pre-renderizar.",
    "editor.err.preRender": "Error durante el pre-renderizado: ",
    "editor.err.imagesOnly": "Por favor, solo suelta archivos de imagen.",
    "editor.err.uploadImages": "Error al subir las imágenes. Por favor, intenta nuevamente.",
    "editor.err.audioOnly": "Por favor, suelta archivos de audio válidos.",
    "editor.err.uploadMusic": "Error al subir música. Intenta nuevamente.",
    "editor.err.uploadSounds": "Error al subir sonidos. Intenta nuevamente.",
    "editor.err.uploadVoice": "Error al subir voz. Intenta nuevamente.",
    "editor.err.deleteMusicFailed": "No se pudo eliminar la música.",
    "editor.err.deleteMusic": "Error al eliminar música.",
    "editor.err.deleteVoiceFailed": "No se pudo eliminar la voz.",
    "editor.err.deleteVoice": "Error al eliminar voz.",
    "editor.err.deleteSoundFailed": "No se pudo eliminar el sonido.",
    "editor.err.deleteSound": "Error al eliminar sonido.",

    // Editor — tooltips
    "editor.tip.deleteMusic": "Eliminar música",
    "editor.tip.deleteScene": "Eliminar escena",
    "editor.tip.deleteElement": "Eliminar elemento",
    "editor.tip.deleteVoice": "Eliminar voz",
    "editor.tip.deleteSound": "Eliminar sonido",
    "editor.tip.changeStart": "Cambiar inicio",
    "editor.tip.changeDuration": "Cambiar duración",

    // Crear frame
    "frame.err.promptAndRatio": "Por favor, ingresa un prompt y selecciona el aspect ratio.",

    // Sugerencias
    "suggestions.err.send": "Error al enviar la sugerencia",

    // Onboarding
    "onboarding.title": "Cuéntanos sobre tu negocio",
    "onboarding.subtitle": "Reelmotion está hecho para equipos y marcas. Esto nos ayuda a configurar tu cuenta.",
    "onboarding.step": "Paso",
    "onboarding.of": "de",
    "onboarding.back": "Atrás",
    "onboarding.next": "Siguiente",
    "onboarding.submit": "Finalizar",
    "onboarding.submitting": "Guardando...",
    "onboarding.err.generic": "No pudimos guardar tus respuestas. Intenta de nuevo.",
    "onboarding.err.url": "Ingresa una URL válida (ej. https://tuempresa.com)",
    "onboarding.company": "Nombre de tu empresa o marca",
    "onboarding.company.ph": "Acme Studios",
    "onboarding.website": "Sitio web o LinkedIn de la empresa",
    "onboarding.website.ph": "https://tuempresa.com",
    "onboarding.country": "País",
    "onboarding.country.ph": "Selecciona tu país",
    "onboarding.role": "¿Cuál es tu rol?",
    "onboarding.role.marketing": "Marketing / Growth",
    "onboarding.role.agency": "Agencia",
    "onboarding.role.founder": "Founder / CEO",
    "onboarding.role.creator": "Creador de contenido",
    "onboarding.role.production": "Producción / Video",
    "onboarding.role.other": "Otro",
    "onboarding.team_size": "Tamaño de tu equipo",
    "onboarding.team_size.1": "Solo yo",
    "onboarding.team_size.2-10": "2-10",
    "onboarding.team_size.11-50": "11-50",
    "onboarding.team_size.51-200": "51-200",
    "onboarding.team_size.200+": "200+",
    "onboarding.use_case": "¿Para qué usarás Reelmotion?",
    "onboarding.use_case.ads": "Anuncios pagados",
    "onboarding.use_case.social": "Redes sociales de mi marca",
    "onboarding.use_case.product": "Videos de producto / e-commerce",
    "onboarding.use_case.agency_clients": "Contenido para clientes de agencia",
    "onboarding.use_case.personal": "Uso personal",
    "onboarding.monthly_volume": "¿Cuántos videos produces al mes?",
    "onboarding.monthly_volume.<5": "Menos de 5",
    "onboarding.monthly_volume.5-20": "5-20",
    "onboarding.monthly_volume.20-100": "20-100",
    "onboarding.monthly_volume.100+": "100+",
    "onboarding.budget": "Presupuesto mensual en contenido de video",
    "onboarding.budget.0": "$0",
    "onboarding.budget.<100": "Menos de $100",
    "onboarding.budget.100-500": "$100 - $500",
    "onboarding.budget.500-2k": "$500 - $2,000",
    "onboarding.budget.2k+": "$2,000+",
    "onboarding.source": "¿Cómo nos conociste?",
    "onboarding.source.google": "Google",
    "onboarding.source.social": "Redes sociales",
    "onboarding.source.referral": "Recomendación",
    "onboarding.source.product_hunt": "Product Hunt",
    "onboarding.source.other": "Otro",
    "onboarding.done.granted.title": "¡Todo listo!",
    "onboarding.done.granted.body": "Te acreditamos 20 tokens de bienvenida para que empieces a crear.",
    "onboarding.done.granted.cta": "Empezar a crear",
    "onboarding.done.title": "Tu cuenta está lista",
    "onboarding.done.body": "Para generar imágenes y videos necesitas tokens. Compra un paquete y empieza hoy.",
    "onboarding.done.buy": "Comprar tokens",
    "onboarding.done.explore": "Explorar la app",
  },
};

export default translations;
